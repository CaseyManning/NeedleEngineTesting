var fs$1 = require('fs');
var micromatch$2 = require('micromatch');
var nodeGzip = require('node-gzip');
var core = require('@caporal/core');
var core$1 = require('@gltf-transform/core');
...
var languageTags = require('language-tags');
var validateSPDX = require('spdx-correct');
var fs$2 = require('fs/promises');
var path = require('path');
var squoosh = require('@squoosh/lib');
...
var micromatch__default = /*#__PURE__*/_interopDefaultLegacy(micromatch$2);
var CLITable__default = /*#__PURE__*/_interopDefaultLegacy(CLITable);
var _commandExists__default = /*#__PURE__*/_interopDefaultLegacy(_commandExists);
var inquirer__default = /*#__PURE__*/_interopDefaultLegacy(inquirer);
var validateSPDX__default = /*#__PURE__*/_interopDefaultLegacy(validateSPDX);
var fs__default$1 = /*#__PURE__*/_interopDefaultLegacy(fs$2);
var path__default = /*#__PURE__*/_interopDefaultLegacy(path);
var squoosh__namespace = /*#__PURE__*/_interopNamespace(squoosh);
...
exports.spawn = _spawn; // See https://github.com/mathisonian/command-exists/issues/22

exports.commandExists = cmd => _commandExists__default['default'](cmd).catch(() => false);
...
async function _waitExit(process) {
  ...
  if (process.stdout) {
    ...
    try {
      for (var _iterator = _asyncIterator(process.stdout), _step, _value; _step = await _iterator.next(), _iteratorNormalCompletion = _step.done, _value = await _step.value, !_iteratorNormalCompletion; _iteratorNormalCompletion = true) {
        const chunk = _value;
        stdout += chunk;
      }
    } catch (err) {
  ...
  if (process.stderr) {
    ...
    try {
      for (var _iterator2 = _asyncIterator(process.stderr), _step2, _value2; _step2 = await _iterator2.next(), _iteratorNormalCompletion2 = _step2.done, _value2 = await _step2.value, !_iteratorNormalCompletion2; _iteratorNormalCompletion2 = true) {
        const chunk = _value2;
        stderr += chunk;
      }
    } catch (err) {
...
async function inspect(jsonDoc, io, logger, format) {
  ...
  const rootPacket = doc.getRoot().getExtension('KHR_xmp_json_ld');
  ...
  const report = functions.inspect(doc);
  ...
  switch (format) {
    ...
    case InspectFormat.CSV:
      return new Promise((resolve, reject) => {
        csvStringify.stringify([head, ...rows], (err, output) => {
...
function getFootnotes(type, rows, header) {
  ...
  if (type === 'textures') {
    ...
    footnotes.push('¹ gpuSize estimates minimum GPU memory allocation. Older devices may require\n' + '  additional memory for GPU compression formats.');
  }
...
function ktxfix() {
  return async doc => {
    ...
    for (const texture of doc.getRoot().listTextures()) {
      ...
      if (!image) continue;
      const ktx = ktxParse.read(image);
      const dfd = ktx.dataFormatDescriptor[0];
      const slots = functions.listTextureSlots(doc, texture); // Don't make changes if we have no information.

      if (slots.length === 0) continue;
      const isLinear = !slots.find(slot => micromatch.isMatch(slot, '*{color,emissive}*', MICROMATCH_OPTIONS));
      const colorPrimaries = isLinear ? ktxParse.KHR_DF_PRIMARIES_UNSPECIFIED : ktxParse.KHR_DF_PRIMARIES_BT709;
      const name = texture.getURI() || texture.getName();
      let changed = false; // See: https://github.com/donmccurdy/glTF-Transform/issues/218
      ...
      if (changed) {
        texture.setImage(ktxParse.write(ktx));
        numChanged++;
      }
    }
...
const merge = options => {
  ...
  return async doc => {
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      const basename = core$1.FileUtils.basename(path);
      const extension = core$1.FileUtils.extension(path).toLowerCase();

      if (['png', 'jpg', 'jpeg', 'webp', 'ktx2'].includes(extension)) {
        doc.createTexture(basename).setImage(fs__default['default'].readFileSync(path)).setMimeType(core$1.ImageUtils.extensionToMimeType(extension)).setURI(basename + '.' + extension);
    ...
    if (!options.partition) {
      await doc.transform(functions.unpartition());
    }
...
const micromatch$1 = require('micromatch');
...
const ETC1S_DEFAULTS = _extends({
  quality: 128,
  compression: 1
}, GLOBAL_DEFAULTS);
const UASTC_DEFAULTS = _extends({
  level: 2,
  rdo: 0,
...
const toktx = function (options) {
  options = _extends({}, options.mode === Mode.ETC1S ? ETC1S_DEFAULTS : UASTC_DEFAULTS, options);
  return async doc => {
    const logger = doc.getLogger(); // Confirm recent version of KTX-Software is installed.

    const version = await checkKTXSoftware(logger);
    const basisuExtension = doc.createExtension(extensions.TextureBasisu).setRequired(true);
    let numCompressed = 0;
    ...
    const promises = textures.map((texture, textureIndex) => limit(async () => {
      const slots = functions.listTextureSlots(doc, texture);
      const channels = functions.getTextureChannelMask(doc, texture);
      ...
      if (texture.getMimeType() === 'image/ktx2') {
        ...
        return;
      } else if (options.slots !== '*' && !slots.find(slot => micromatch$1.isMatch(slot, options.slots, MICROMATCH_OPTIONS))) {
      ...
      const extension = texture.getURI() ? core$1.FileUtils.extension(texture.getURI()) : core$1.ImageUtils.mimeTypeToExtension(texture.getMimeType());
      ...
      const params = [...createParams(slots, channels, size, logger, numTextures, options, version), outPath, inPath];
      logger.debug(`${prefix}: Spawning → toktx ${params.join(' ')}`); // COMPRESS: Run `toktx` CLI tool.
      // eslint-disable-next-line @typescript-eslint/no-unused-vars

      const [status, stdout, stderr] = await exports.waitExit(exports.spawn('toktx', params));
      ...
      if (status !== 0) {
        ...
        if (texture.getURI()) {
          texture.setURI(core$1.FileUtils.basename(texture.getURI()) + '.ktx2');
        }
...
function createParams(slots, channels, size, logger, numTextures, options, version) {
  ...
  if (slots.find(slot => micromatch$1.isMatch(slot, '*normal*', MICROMATCH_OPTIONS))) {
  ...
  if (slots.length && !slots.find(slot => micromatch$1.isMatch(slot, '*{color,emissive}*', MICROMATCH_OPTIONS))) {
...
async function checkKTXSoftware(logger) {
  if (!(await exports.commandExists('toktx')) && !process.env.CI) {
    throw new Error('Command "toktx" not found. Please install KTX-Software, from:\n\nhttps://github.com/KhronosGroup/KTX-Software');
  }

  const [status, stdout, stderr] = await exports.waitExit(exports.spawn('toktx', ['--version']));
...
function generateQuestions(_x) {
  return _generateQuestions.apply(this, arguments);
}

function _generateQuestions() {
  _generateQuestions = _wrapAsyncGenerator(function* (results) {
    ...
    const prompts = new Set(results._prompts); // Prompt for 'dc:language' first, used as the default for Language Alternative entries.
    ...
    if (prompts.has(Prompt.RIGHTS)) {
      yield {
        ...
        loop: false,
        pageSize: 15,
        choices: [// Common SPDX license identifiers applicable to creative works.
        {
          value: '',
          name: 'Unspecified'
        }, {
      ...
      if (results._rights === 'OTHER') {
        ...
        if (results._isLicenseSPDX) {
          yield {
            ...
            validate: input => validateSPDX__default['default'](input) ? true : 'Invalid SPDX ID; refer to https://spdx.dev/.'
          };
        } else {
          yield {
            type: 'input',
...
const xmp = (_options = XMP_DEFAULTS) => {
  const options = _extends({}, XMP_DEFAULTS, _options);

  return async document => {
    const logger = document.getLogger();
    const root = document.getRoot();
    const xmpExtension = document.createExtension(extensions.XMP);
    ...
    if (options.packet) {
      const packetPath = path__default['default'].resolve(options.packet);
      logger.info(`[xmp]: Loading "${packetPath}"...`);
      const packetJSON = await fs__default$1['default'].readFile(packetPath, 'utf-8');
    ...
    try {
      ...
      try {
        for (var _iterator = _asyncIterator(generateQuestions(results)), _step, _value; _step = await _iterator.next(), _iteratorNormalCompletion = _step.done, _value = await _step.value, !_iteratorNormalCompletion; _iteratorNormalCompletion = true) {
          const question = _value;
    ...
    packet.setContext(_extends({}, packet.getContext(), createContext(results), {
...
class Session {
  constructor(_io, _logger, _input, _output) {
    ...
    this._outputFormat = core$1.FileUtils.extension(_output) === 'glb' ? core$1.Format.GLB : core$1.Format.GLTF;
  }
  ...
  async transform(...transforms) {
    const doc = this._input ? (await this._io.read(this._input)).setLogger(this._logger) : new core$1.Document().setLogger(this._logger); // Warn and remove lossy compression, to avoid increasing loss on round trip.

    for (const extensionName of ['KHR_draco_mesh_compression', 'EXT_meshopt_compression']) {
      const extension = doc.getRoot().listExtensionsUsed().find(extension => extension.extensionName === extensionName);
    ...
    if (this._outputFormat === core$1.Format.GLB) {
      await doc.transform(functions.unpartition());
    }
    ...
    if (!this._input) {
      const output = core$1.FileUtils.basename(this._output) + '.' + core$1.FileUtils.extension(this._output);

      this._logger.info(`${output} (${formatBytes(lastWriteBytes)})`);
    } else {
      const input = core$1.FileUtils.basename(this._input) + '.' + core$1.FileUtils.extension(this._input);
      const output = core$1.FileUtils.basename(this._output) + '.' + core$1.FileUtils.extension(this._output);
...
function updateMetadata(document) {
  const root = document.getRoot();
  const xmpExtension = root.listExtensionsUsed().find(ext => ext.extensionName === 'KHR_xmp_json_ld'); // Do not add KHR_xmp_json_ld to assets that don't already use it.

  if (!xmpExtension) return;
  const rootPacket = root.getExtension('KHR_xmp_json_ld') || xmpExtension.createPacket(); // xmp:MetadataDate should be the same as, or more recent than, xmp:ModifyDate.
  // https://github.com/adobe/xmp-docs/blob/master/XMPNamespaces/xmp.md

  const date = new Date().toISOString().substring(0, 10);
  rootPacket.setContext(_extends({}, rootPacket.getContext(), {
    xmp: XMPContext.xmp
...
function validate(input, options, logger) {
  const buffer = fs__default['default'].readFileSync(input);
  return validator.validateBytes(new Uint8Array(buffer), {
    ...
    externalResourceFunction: uri => new Promise((resolve, reject) => {
      uri = path__default['default'].resolve(path__default['default'].dirname(input), decodeURIComponent(uri));
      fs__default['default'].readFile(uri, (err, data) => {
...
function printIssueSection(header, severity, report, logger) {
  ...
  if (messages.length) {
    const table = new CLITable__default['default']({
      head: ['code', 'message', 'severity', 'pointer']
    });
...
let io; // Use require() so microbundle doesn't compile these.
...
const programReady = new Promise(resolve => {
  Promise.all([draco3d.createDecoderModule(), draco3d.createEncoderModule(), MeshoptDecoder.ready, MeshoptEncoder.ready]).then(([decoder, encoder, _]) => {
    io = new core$1.NodeIO(fetch).registerExtensions(extensions.ALL_EXTENSIONS).registerDependencies({
...
core.program.version(require('../package.json').version).description('Commandline interface for the glTF-Transform SDK.');
core.program.command('', '\n\n🔎 INSPECT ──────────────────────────────────────────'); // INSPECT

core.program.command('inspect', 'Inspect the contents of the model').help(`
...
core.program.command('validate', 'Validate the model against the glTF spec').help(`
...
Example:
  ...
  validator: core.program.NUMBER,
  default: 1e7
}).option('--ignore <CODE>,<CODE>,...', 'Issue codes to be ignored', {
  validator: core.program.ARRAY,
  default: []
}).action(({
  args,
  options,
...
core.program.command('copy', 'Copy the model with minimal changes').alias('cp').help(`
...
`.trim()).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).action(({
  args,
  logger
}) => Session.create(io, logger, args.input, args.output).transform()); // MERGE

core.program.command('merge', 'Merge two or more models into one').help(`
...
Example:
  ...
  validator: core.program.BOOLEAN,
  default: false
}).action(({
  args,
  options,
...
core.program.command('partition', 'Partition binary data into separate .bin files').help(`
...
resources as needed. Partitioning is supported only for .gltf, not .glb, files.
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.partition(options))); // DEDUP

core.program.command('dedup', 'Deduplicate accessors and textures').help(`
...
compression and instancing, to be more effective.
  ...
  const propertyTypes = [];
  if (options.accessors) propertyTypes.push(core$1.PropertyType.ACCESSOR);
  if (options.materials) propertyTypes.push(core$1.PropertyType.MATERIAL);
  if (options.meshes) propertyTypes.push(core$1.PropertyType.MESH);
  if (options.textures) propertyTypes.push(core$1.PropertyType.TEXTURE);
  return Session.create(io, logger, args.input, args.output).transform(functions.dedup({
    propertyTypes
  }));
}); // PRUNE

core.program.command('prune', 'Remove unreferenced properties from the file').help(`
...
that are children of a scene.
	`.trim()).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--keep-leaves <keepLeaves>', 'Whether to keep empty leaf nodes', {
  validator: core.program.BOOLEAN,
  default: false
}).action(({
  args,
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.prune(options))); // GZIP

core.program.command('gzip', 'Compress the model with lossless gzip').help(`
...
`).argument('<input>', INPUT_DESC).action(async ({
  ...
  const inBuffer = await fs$1.promises.readFile(args.input);
  const outBuffer = await nodeGzip.gzip(inBuffer);
  const fileName = args.input + '.gz';
  const inSize = formatBytes(inBuffer.byteLength);
  const outSize = formatBytes(outBuffer.byteLength);
  await fs$1.promises.writeFile(fileName, outBuffer);
  logger.info(`Created ${fileName} (${inSize} → ${outSize})`);
}); // XMP

core.program.command('xmp', 'Add or modify XMP metadata').help(`
...
`).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--packet <path>', 'Path to XMP packet (.jsonld or .json)').option('--reset', 'Reset metadata and remove XMP extension', {
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(xmp(_extends({}, options))));
core.program.command('', '\n\n🌍 SCENE ────────────────────────────────────────────'); // CENTER

core.program.command('center', 'Center the scene at the origin, or above/below it').help(`
...
attached a surface, like a ceiling fan, the pivot may be located above instead.
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.center(_extends({}, options)))); // INSTANCE

core.program.command('instance', 'Create GPU instances from shared Mesh references').help(`
...
https://github.com/KhronosGroup/glTF/tree/master/extensions/2.0/Vendor/EXT_mesh_gpu_instancing.
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.instance(_extends({}, options))));
core.program.command('', '\n\n🕋 GEOMETRY ─────────────────────────────────────────'); // DRACO

core.program.command('draco', 'Compress geometry with Draco').help(`
...
`.trim()).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--method <method>', 'Compression method.', {
  ...
  validator: core.program.NUMBER,
  default: functions.DRACO_DEFAULTS.encodeSpeed
}).option('--decode-speed <decodeSpeed>', 'Decoding speed vs. compression level, 1–10.', {
  validator: core.program.NUMBER,
  default: functions.DRACO_DEFAULTS.decodeSpeed
}).option('--quantize-position <bits>', 'Quantization bits for POSITION, 1-16.', {
  validator: core.program.NUMBER,
  default: functions.DRACO_DEFAULTS.quantizePosition
}).option('--quantize-normal <bits>', 'Quantization bits for NORMAL, 1-16.', {
  validator: core.program.NUMBER,
  default: functions.DRACO_DEFAULTS.quantizeNormal
}).option('--quantize-color <bits>', 'Quantization bits for COLOR_*, 1-16.', {
  validator: core.program.NUMBER,
  default: functions.DRACO_DEFAULTS.quantizeColor
}).option('--quantize-texcoord <bits>', 'Quantization bits for TEXCOORD_*, 1-16.', {
  validator: core.program.NUMBER,
  default: functions.DRACO_DEFAULTS.quantizeTexcoord
}).option('--quantize-generic <bits>', 'Quantization bits for other attributes, 1-16.', {
  validator: core.program.NUMBER,
  default: functions.DRACO_DEFAULTS.quantizeGeneric
}).option('--quantization-volume <volume>', 'Bounds for quantization grid.', {
  validator: ['mesh', 'scene'],
  default: functions.DRACO_DEFAULTS.quantizationVolume
}).action(({
  args,
  options,
  logger
}) => // Include a lossless weld — Draco requires indices.
Session.create(io, logger, args.input, args.output).transform(functions.weld({
  tolerance: 0
}), functions.draco(options))); // MESHOPT

core.program.command('meshopt', 'Compress geometry and animation with Meshopt').help(`
...
`.trim()).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--level <level>', 'Compression level.', {
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.meshopt(_extends({
  encoder: MeshoptEncoder
}, options)))); // QUANTIZE

core.program.command('quantize', 'Quantize geometry, reducing precision and memory').help(`
...
Requires KHR_mesh_quantization support.`.trim()).argument('<input>', 'Path to read glTF 2.0 (.glb, .gltf) input').argument('<output>', 'Path to write output').option('--pattern <pattern>', 'Pattern for vertex attributes (case-insensitive glob)', {
  validator: core.program.STRING,
  default: '*'
}).option('--quantize-position <bits>', 'Precision for POSITION attributes.', {
  validator: core.program.NUMBER,
  default: functions.QUANTIZE_DEFAULTS.quantizePosition
}).option('--quantize-normal <bits>', 'Precision for NORMAL and TANGENT attributes.', {
  validator: core.program.NUMBER,
  default: functions.QUANTIZE_DEFAULTS.quantizeNormal
}).option('--quantize-texcoord <bits>', 'Precision for TEXCOORD_* attributes.', {
  validator: core.program.NUMBER,
  default: functions.QUANTIZE_DEFAULTS.quantizeTexcoord
}).option('--quantize-color <bits>', 'Precision for COLOR_* attributes.', {
  validator: core.program.NUMBER,
  default: functions.QUANTIZE_DEFAULTS.quantizeColor
}).option('--quantize-weight <bits>', 'Precision for WEIGHTS_* attributes.', {
  validator: core.program.NUMBER,
  default: functions.QUANTIZE_DEFAULTS.quantizeWeight
}).option('--quantize-generic <bits>', 'Precision for custom (_*) attributes.', {
  validator: core.program.NUMBER,
  default: functions.QUANTIZE_DEFAULTS.quantizeGeneric
}).option('--quantization-volume <volume>', 'Bounds for quantization grid.', {
  validator: ['mesh', 'scene'],
  default: functions.QUANTIZE_DEFAULTS.quantizationVolume
}).action(({
  args,
  options,
  logger
}) => {
  const pattern = micromatch__default['default'].makeRe(String(options.pattern), MICROMATCH_OPTIONS);
  return Session.create(io, logger, args.input, args.output).transform(functions.quantize(_extends({}, options, {
    pattern
  })));
}); // DEQUANTIZE

core.program.command('dequantize', 'Dequantize geometry').help(`
...
Removes KHR_mesh_quantization, if present.`.trim()).argument('<input>', 'Path to read glTF 2.0 (.glb, .gltf) input').argument('<output>', 'Path to write output').option('--pattern <pattern>', 'Pattern for vertex attributes (case-insensitive glob)', {
  ...
  const pattern = micromatch__default['default'].makeRe(String(options.pattern), MICROMATCH_OPTIONS);
  return Session.create(io, logger, args.input, args.output).transform(functions.dequantize(_extends({}, options, {
    pattern
  })));
}); // WELD

core.program.command('weld', 'Index geometry and optionally merge similar vertices').help(`
...
is indexed in place, without merging.
	`.trim()).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--tolerance', 'Tolerance for vertex welding', {
  validator: core.program.NUMBER,
  default: functions.WELD_DEFAULTS.tolerance
}).action(({
  args,
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.weld(options))); // UNWELD

core.program.command('unweld', 'De-index geometry, disconnecting any shared vertices').help(`
...
paricular software application.
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.unweld(options))); // TANGENTS

core.program.command('tangents', 'Generate MikkTSpace vertex tangents').help(`
...
compute MikkTSpace tangents at runtime.
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.tangents(_extends({
  generateTangents: mikktspace.generateTangents
}, options)))); // REORDER

core.program.command('reorder', 'Optimize vertex data for locality of reference').help(`
...
Based on the meshoptimizer library (https://github.com/zeux/meshoptimizer).
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.reorder(_extends({
  encoder: MeshoptEncoder
}, options)))); // SIMPLIFY

core.program.command('simplify', 'Simplify mesh, reducing number of vertices').help(`
...
`.trim()).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--ratio <ratio>', 'Target ratio (0–1) of vertices to keep', {
  validator: core.program.NUMBER,
  default: functions.SIMPLIFY_DEFAULTS.ratio
}).option('--error <error>', 'Target error, as a fraction of mesh radius', {
  validator: core.program.NUMBER,
  default: functions.SIMPLIFY_DEFAULTS.error
}).option('--lock-border <lockBorder>', 'Whether to lock topological borders of the mesh', {
  validator: core.program.BOOLEAN,
  default: functions.SIMPLIFY_DEFAULTS.lockBorder
}).action(async ({
  args,
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.simplify(_extends({
  simplifier: MeshoptSimplifier
}, options))));
core.program.command('', '\n\n✨ MATERIAL ─────────────────────────────────────────'); // METALROUGH

core.program.command('metalrough', 'Convert materials from spec/gloss to metal/rough').help(`
...
should be used as input, and then compressed after this conversion.
  ...
  args,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.metalRough())); // UNLIT

core.program.command('unlit', 'Convert materials from metal/rough to unlit').help(`
...
Unlit materials are also helpful for non-physically-based visual styles.
  ...
  args,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.unlit()));
core.program.command('', '\n\n🖼  TEXTURE ──────────────────────────────────────────'); // RESIZE

core.program.command('resize', 'Resize PNG or JPEG textures').help(`
...
`.trim()).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--pattern <pattern>', 'Pattern (regex) to match textures, by name or URI.', {
  validator: core.program.STRING
}).option('--filter', 'Resampling filter', {
  validator: [functions.TextureResizeFilter.LANCZOS3, functions.TextureResizeFilter.LANCZOS2],
  default: functions.TEXTURE_RESIZE_DEFAULTS.filter
  ...
  const pattern = options.pattern ? micromatch__default['default'].makeRe(String(options.pattern), MICROMATCH_OPTIONS) : null;
  return Session.create(io, logger, args.input, args.output).transform(functions.textureResize({
...
core.program.command('etc1s', 'KTX + Basis ETC1S texture compression').help(BASIS_SUMMARY.replace('{VARIANT}', 'ETC1S').replace('{DETAILS}', `
...
normal maps and ETC1S for other textures, for example.`.trim()), {
  sectionName: 'SUMMARY'
}).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--slots <slots>', 'Texture slots to include (glob)', {
  validator: core.program.STRING,
  default: '*'
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(toktx(_extends({
  mode: Mode.ETC1S
}, options)))); // UASTC

core.program.command('uastc', 'KTX + Basis UASTC texture compression').help(BASIS_SUMMARY.replace('{VARIANT}', 'UASTC').replace('{DETAILS}', `
...
core.program.command('ktxfix', 'Fixes common issues in KTX texture metadata').help(`
...
core.program.command('webp', 'WebP texture compression').help(SQUOOSH_SUMMARY.replace(/{VARIANT}/g, 'WebP')).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--formats <formats>', 'Texture formats to include (glob)', {
  validator: ['image/png', 'image/jpeg', '*'],
  default: '*'
}).option('--slots <slots>', 'Texture slots to include (glob)', {
  validator: core.program.STRING,
  default: '*'
}).option('--auto', 'Enables experimental auto-optimization with perceptual metrics.' + ' Slower, with typically higher-quality and larger images' + ' compared to the default encoder settings.', {
  validator: core.program.BOOLEAN,
  default: false
}).action(({
  args,
  ...
  const formats = micromatch__default['default'].makeRe(String(options.formats), MICROMATCH_OPTIONS);
  const slots = micromatch__default['default'].makeRe(String(options.slots), MICROMATCH_OPTIONS);
  return Session.create(io, logger, args.input, args.output).transform(functions.webp(_extends({}, options, {
    formats,
    slots,
    squoosh: squoosh__namespace
  })));
}); // OXIPNG

core.program.command('oxipng', 'OxiPNG texture compression').help(SQUOOSH_SUMMARY.replace(/{VARIANT}/g, 'OxiPNG')).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--formats <formats>', 'Texture formats to include (glob)', {
  validator: ['image/png', 'image/jpeg', '*'],
  default: 'image/png'
}).option('--slots <slots>', 'Texture slots to include (glob)', {
  validator: core.program.STRING,
  ...
  const formats = micromatch__default['default'].makeRe(String(options.formats), MICROMATCH_OPTIONS);
  const slots = micromatch__default['default'].makeRe(String(options.slots), MICROMATCH_OPTIONS);
  return Session.create(io, logger, args.input, args.output).transform(functions.oxipng(_extends({}, options, {
    formats,
    slots,
    squoosh: squoosh__namespace
  })));
...
core.program.command('mozjpeg', 'MozJPEG texture compression').help(SQUOOSH_SUMMARY.replace(/{VARIANT}/g, 'MozJPEG')).argument('<input>', INPUT_DESC).argument('<output>', OUTPUT_DESC).option('--formats <formats>', 'Texture formats to include (glob)', {
  validator: ['image/png', 'image/jpeg', '*'],
  default: 'image/jpeg'
}).option('--slots <slots>', 'Texture slots to include (glob)', {
  validator: core.program.STRING,
  ...
  const formats = micromatch__default['default'].makeRe(String(options.formats), MICROMATCH_OPTIONS);
  const slots = micromatch__default['default'].makeRe(String(options.slots), MICROMATCH_OPTIONS);
  return Session.create(io, logger, args.input, args.output).transform(functions.mozjpeg(_extends({}, options, {
    formats,
    slots,
    squoosh: squoosh__namespace
  })));
...
core.program.command('resample', 'Resample animations, losslessly deduplicating keyframes').help(`
...
--tolerance, and should have no visible effect on animation playback.
  ...
  options,
  logger
}) => Session.create(io, logger, args.input, args.output).transform(functions.resample(options))); // SEQUENCE

core.program.command('sequence', 'Animate node visibilities as a flipboard sequence').help(`
...
so this workflow is not a replacement for video playback.
  ...
  const pattern = micromatch__default['default'].makeRe(String(options.pattern), MICROMATCH_OPTIONS);
  return Session.create(io, logger, args.input, args.output).transform(functions.sequence(_extends({}, options, {
    pattern
  })));
});
core.program.option('--allow-http', 'Allows reads from HTTP requests.', {
  global: true,
  default: false,
  validator: core.program.BOOLEAN,
  action: ({
...
core.program.option('--vertex-layout <layout>', 'Vertex buffer layout preset.', {
  global: true,
  default: core$1.VertexLayout.INTERLEAVED,
  validator: [core$1.VertexLayout.INTERLEAVED, core$1.VertexLayout.SEPARATE],
  action: ({
    options
  }) => {
...
core.program.disableGlobalOption('--quiet');
core.program.disableGlobalOption('--no-color');
...
exports.ETC1S_DEFAULTS = ETC1S_DEFAULTS;
exports.Filter = Filter;
exports.MICROMATCH_OPTIONS = MICROMATCH_OPTIONS;
exports.Mode = Mode;
exports.UASTC_DEFAULTS = UASTC_DEFAULTS;
exports.XMPContext = XMPContext;
exports.XMP_DEFAULTS = XMP_DEFAULTS;
